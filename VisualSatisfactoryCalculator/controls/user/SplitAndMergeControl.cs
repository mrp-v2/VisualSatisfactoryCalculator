...
namespace VisualSatisfactoryCalculator.controls.user
{
	public partial class SplitAndMergeControl : UserControl, ILayoutControl
	{
		...
		public SplitAndMergeControl(Connection backingConnection, MainForm mainForm)
		{
			...
			{
				ItemRateControl irc = new ItemRateControl(mainForm, BackingConnection.ItemID, backingConnection.GetProducerRate(step), false, 3, RateChanged, ItemClicked);
			...
			{
				ItemRateControl irc = new ItemRateControl(mainForm, BackingConnection.ItemID, BackingConnection.GetConsumerRate(step), true, 3, RateChanged, ItemClicked);
				irc.ItemButton.Enabled = false;
